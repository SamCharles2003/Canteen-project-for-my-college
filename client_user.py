...
def check_user():
    ...
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        if user:
            ...
            return redirect(url_for('students_dashboard', email=email))
        else:
...
def current_balance_return():
    try:
        email=session['email']
        cursor.execute("SELECT`current_balance` FROM `rfid_data` WHERE `mail_id`= %s ",args=(email))
        variable=cursor.fetchall()[0]
        print("CURRENT_BALANCE()--->",variable)
        return variable
    except Exception as e:
        print("CURRENT_BALANCE()--->ERROR:",e)

def current_purchase_return():
    try:


@app.route('/students_dashboard',methods=['POST'])
